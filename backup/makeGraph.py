...
def makeRootGraph(index):
    ...
    while True:
        if B==0:break
        logData.rootG.add_weighted_edges_from([(str(A), str(B), logData.incoming[B][A])])#"%d/%d"%(logData.incoming[B][A] if logData.incoming[B][A]!=0 else logData.outgoing[A][B],
...
def makeNeighborGraph(index):
    ...
    for i in range(1, logData.NODE_CNT+1):
        ...
        if logData.outgoing[index][i]==0 or logData.incoming[index][i]==0:
            continue
        logData.neighborG.add_edge(str(index), str(i), weight=logData.incoming[index][i] if logData.incoming[index][i]!=0 else logData.outgoing[i][index])
