...
pressX, pressY, pressNode = None, None, None

def nodeRelease(event):
    releaseNode = 0
    if mode.get()==2:return
    global canvas_widget, label, node_number, pressX, pressY, pressNode
    if event.button == 1:
        for key in logData.pos.keys():
            ...
            if (nodex-event.xdata)**2+(nodey-event.ydata)**2<=0.002:
                ...
                if mode.get()==0:
                    ...
                    drawNeighborGraph()   
                releaseNode = pressNode
                break
        print("press :", pressNode, "release :", releaseNode)
        if releaseNode == pressNode:
            changeLabel(releaseNode)
            if mode.get()==0:
                makeGraph.makeRootGraph(releaseNode)
                drawRootGraph(logData.rootG)
            elif mode.get()==1:
                makeGraph.makeNeighborGraph(releaseNode)
                drawNeighborGraph()
        elif releaseNode==0 and pressNode!=0:
            logData.pos[str(pressNode)] = [event.xdata, event.ydata]
            if mode.get()==0:
                drawRootGraph(releaseNode)
            elif mode.get()==1:
                drawNeighborGraph()
            elif mode.get()==2:
...
def nodePress(event):
    global pressX, pressY, pressNode
    if event.button == 1:
        pressX = event.xdata
        pressY = event.ydata
        for key in logData.pos.keys():
...
def drawNeighborGraph():
    ...
    fig.canvas.mpl_connect("button_press_event",nodePress)
    fig.canvas.mpl_connect("button_release_event",nodeRelease)
